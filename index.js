...
async function doLogin(url) {
  ...
  await page.waitForSelector(
    "h3.timeline-comment-header-text .js-timestamp relative-time"
  );

  const pr = await page.$eval(".gh-header-title span:last-child", (pr) =>
    pr && pr.textContent ? pr.textContent : ""
  );
  const prAuthor = await page.$eval(".gh-header-meta a.author", (author) =>
    author && author.textContent ? author.textContent : ""
  );
  const prTitle = await page.$eval(
    ".gh-header-title .js-issue-title",
    (title) => (title && title.outerText ? title.outerText : "")
  );
  const prCreatedAt = await page.$eval(
    "h3.timeline-comment-header-text .js-timestamp relative-time",
    (createdAt) => {
      if (createdAt) {
  ...
  // return discussion
  const discussion = await page.$$eval(
    ".js-discussion .js-timeline-item",
    (createdAt) => {
      return createdAt.map((el) => {
        if (el.querySelector(".TimelineItem-body relative-time")) {
          const time = el
            .querySelector(".TimelineItem-body relative-time")
            .getAttribute("datetime");
          const author =
            el.querySelector(".TimelineItem-body a.author") &&
  ...
  const length2 = await page.$$eval(
    ".js-discussion .js-timeline-item .TimelineItem-body relative-time",
    (items) => items.length
  );
  ...
  const pullrequest = {
    ...
    prClosestCommitToFirstReview:
      discussion[indexOfLastCommit] && discussion[indexOfLastCommit].time
        ? moment.utc(discussion[indexOfLastCommit].time).local().format()
        : "",
    prFirstReviewAt:
      prFirstReview && prFirstReview.time
        ? moment.utc(prFirstReview.time).local().format()
        : "",
    prFirstReviewAuthor:
      prFirstReview && prFirstReview.author ? prFirstReview.author : "",
    prTimeToFirstReview: "",
  };
  ...
  if (
    prFirstReview &&
    prFirstReview.time &&
    discussion[indexOfLastCommit] &&
    discussion[indexOfLastCommit].time
  ) {
